...
import {CURRENCY_SYMBOLS} from "@/lib/currencySymbols.ts";
...
export async function fetchExchangeRates(from: string): Promise<ExchangeRates> {
  const resp = await fetch(
    CURRENCY_CONVERSION_ENDPOINT.replace("{{from}}", from.toLowerCase()),
  );
  if (!resp.ok) {
 ...
 */
export async function convertDinero(
  dinero: Dinero,
  to: Currency,
  ...
  if (!exchangeRates) {
    // Store current exchange rates in `sessionStorage` to save on API calls. Catch errors where this method is called
    // from the server.
    let exchangeRatesString;
    try {
      exchangeRatesString = window.sessionStorage.getItem(
        `exchangeRates${dinero.getCurrency()}`,
      );
    } catch (e: unknown) {
  ...
  const rate =
    exchangeRates![dinero.getCurrency().toLowerCase()][to.toLowerCase()];
  const toPrecision =
    CURRENCY_SYMBOLS[to.toUpperCase() as keyof typeof CURRENCY_SYMBOLS]
      ?.precision ?? 2;
